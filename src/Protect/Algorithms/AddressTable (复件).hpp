...
//sort(addr_vec.begin(),addr_vec.end());
  ...
  if (sign)
  {
    //sort(addr_vec.begin(),addr_vec.end(),_less);   //正增长
    for (vector<useful_space>::iterator iter = useful_vec.begin(); iter != useful_vec.end(); ++iter)
    {
      if (iter->end - iter->begin >= _size)
      {
        if (count == r)
        {
          space p;
          ////注释掉这句就只会在某些概率下随机了
          p.begin = rand() % (iter->end - iter->begin - _size) + iter->begin;
          ////
          //p.begin = iter->begin;
  ...
  {
    //sort(addr_vec.begin(),addr_vec.end(),_greater);  //负增长
    for (vector<useful_space>::iterator iter = useful_vec.begin(); iter != useful_vec.end(); ++iter)
    {
      if (iter->end - iter->begin >= _size)
      {
