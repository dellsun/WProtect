...
long BuildVMByteCode::build_exit_vm()
{
            ...
            iter != var_list_code_piece.end();iter++)
    {
                ...
                uiter != iter->get_piece().end();uiter++)
        {
           ...
           if (++uiter_check_end == iter->get_piece().end()) //是否是最后一条指令
           {
              ...
              ///////////退出虚拟机处理
              //if (iter->get_jmplabel_type() == TYPE_ADDR || iter->get_jmplabel_type() == TYPE_MEM)
              if (iter->get_opcode_attribute() == OPCODE_ATTRIBUTE_CALL
                      || (iter->get_opcode_attribute() == OPCODE_ATTRIBUTE_JMP && iter->get_jmplabel() == PC_NONE)
                      || iter->get_opcode_attribute() == OPCODE_ATTRIBUTE_EXIT
                      )
              {
                  //创建下一条指令的入口
                  long next_vmcode_addr = get_vmcode_address(iter->get_label() + 1);
                  long ignore_build_code_addr = iter->get_jmplabel();
                  //var_combos_vm_code.int3();
                  if (next_vmcode_addr == 0)
                      next_vmcode_addr = uiter->pc;
                  if (next_vmcode_addr && b_allocator == false &&
                          (iter->get_opcode_attribute() == OPCODE_ATTRIBUTE_CALL
                           || iter->get_opcode_attribute == OPCODE_ATTRIBUTE_JMP))
                  {
                        //printf("退出虚拟机指令%s\n",uiter->insn_buffer);
                        printf("TYPE_ADDR|TYPE_MEM:%-32s\n",uiter->insn_buffer);
                        AsmJit::Assembler a;
                        //AsmJit::Operand o;
                        if (uiter->operand[0].type == UD_OP_JIMM)
                        {
                            bool t_sign = ptr_addr_table->get_sign();
                            ptr_addr_table->set_sign(true);
                            a.jmp(ignore_build_code_addr);
                            ignore_build_code_addr = ptr_addr_table->assign_address(a.getCodeSize());
                            printf("ignore code addr:%x\n",ignore_build_code_addr);
                            a.relocCode(a.getCode(),ignore_build_code_addr);
                            ptr_addr_table->copy(ignore_build_code_addr,a.getCode(),a.getCodeSize());
                        ...
                        else
                        {
                            ...
                            ignore_build_code_addr = ptr_addr_table->assign_address(ud_insn_len(&t_ud));
                            printf("ignore code addr:%x\n",ignore_build_code_addr);
                            unsigned char * new_data = new unsigned char[ud_insn_len(&t_ud)];
                            size_t length = (size_t)ud_insn_len(&t_ud);
                            memcpy(new_data,ud_insn_ptr(&t_ud),length);
                            //new_data[ud_insn_len(&t_ud)] = 0xc3;
                            if ( ud_insn_mnemonic(&t_ud) == UD_Icall)
                            for (int i = 0;i<length;i++)
                            {
                                //const uint8_t * code_buf = ud_insn_ptr(&t_ud);
                                if (new_data[i] == 0xff)
                                {
                                    uint8_t modrm = new_data[i+1];
                                    //把call变成jmp
                                    if (modrm & ( 1 << 4 ) )
                                    {
                                        //call
                                        modrm = (~(1<<4)) & modrm;
                                        modrm = modrm | (1 << 5); //jmp modrm
                                        new_data[i+1] = modrm;
                                        break;
                                    }
                                }
                            }
                            ptr_addr_table->copy(ignore_build_code_addr,new_data,ud_insn_len(&t_ud));
                            delete [] new_data;
                            ptr_addr_table->set_sign(t_sign);
                            //ptr_addr_table->copy(ignore_build_code_addr,ud_insn_ptr(&t_ud),ud_insn_len(&t_ud));
                            //a.relocCode(a.getCode(),ignore_build_code_addr);


                            /*ud_t & t_ud = *uiter;
                            for (int i = 0;i < ud_insn_len(&t_ud);i++)
                            {
                               const uint8_t * code_buf = ud_insn_ptr(&t_ud);
                               a._emitByte(code_buf[i]);
                            }*/
                        }

                        if (var_map_label_vmbuffer_manage.find(var_cur_label+1) != var_map_label_vmbuffer_manage.end()) {
                        ...
                        ptr_addr_table->set_sign(t_sign);
                        VMCodeBufferManage * next_buffer = var_map_label_vmbuffer_manage[var_cur_label + 1];

                        ppcode_block_info info = ptr_vm->create_function_head(
                                    head_address,
                                    next_vmcode_addr,
                                    &(next_buffer->get_pcode()),
                                    123,
                                    123456,
                                    init_de_key); //pcode位置有问题
                        ptr_addr_table->copy(head_address,info->buf,info->size);
                        printf("VM入口点地址:%x\n",head_address);
                        next_vmcode_addr = head_address;}
                  }
                  //if (uiter->mnemonic == UD_Icall)
                  if (iter->get_opcode_attribute() == OPCODE_ATTRIBUTE_CALL)
                     var_combos_vm_code.d_push_imm(next_vmcode_addr);
                  var_combos_vm_code.d_push_imm(ignore_build_code_addr);
                  //if (uiter->mnemonic != UD_Icall && uiter->mnemonic != UD_Ijmp)
             ...
             {/*{{{*/
                ...
                {
                   long next_addr;
                   next_addr = get_vmcode_address(iter->get_jmplabel());
                   if (next_addr == 0 && iter->get_jmplabel_type() != TYPE_RET)
                   //if (next_addr && iter->get_opcode_attribute() == OPCODE_ATTRIBUTE_EXIT)
                   {
                       printf("没有获取到地址\n");
                       __asm__("int3");
                   }
                   //next_addr =  get_vmcode_address(iter->get_label() + 1);//var_map_label_vmcode_addr[iter->get_label() + 1];
                   var_combos_vm_code.d_push_imm(next_addr);
                }
                var_combos_vm_code.pop(T_NEXTINSADDR);
                build(var_combos_vm_code,*uiter);
           ...
           if (b_allocator == false && uiter->mnemonic != UD_Inop)
           {
#ifdef DEBUG
        ...
        //if (iter->get_jmplabel_type() != TYPE_RET && iter->get_jmplabel_type() != TYPE_ADDR
        //        && iter->get_jmplabel_type() != TYPE_MEM)
        //if (iter->get_jmplabel_type() == TYPE_LABEL)
